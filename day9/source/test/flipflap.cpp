...
struct BLOCK{
	...
	inline int build(int m)
	{
		...
		sz=int(sqrt(n*(log(n)/log(2))/13+1)),bc=(n-1)/sz+1;
	...
	inline int _merge(int l,int p,int r)//[l,p],(p,r]
	{
	...
	//	rep(i,1,Lc) cerr<<a[Lt[i]]sp;cerr ln;
	//	rep(i,1,Rc) cerr<<a[Rt[i]]sp;cerr ln;
		int i=1,j=1,cnt=0;
		while(i<=Lc&&j<=Rc)
	...
	//	rep(i,L[b],R[p]) if(ps[i]>=l&&ps[i]<=r) cerr<<a[ps[i]]sp;cerr ln ln;
		return 0;
	}
	inline int _merge(int L,int p1,int p2,int R)//[L,p1),[p1,p2),[p2,R]
	{
		int Ac=0,Bc=0,Cc=0;
		rep(i,L,R)
	...
	inline int update(int p,lint v)
	{
	//	debug(val)sp,debug(p)sp,debug(v)ln;
		p=nxt[p],assert(p),a[p]+=v;int bp=bel[p];
		_merge(L[bp],p,p+1,R[bp]);
	//	_merge(L[bp],p-1,p),_merge(L[bp],p,R[bp]);
		return 0;
	}
	...
	//	debug(val)sp,debug(s)sp,debug(t)ln;
	//	rep(i,1,n) cerr<<a[i]+tg[bel[i]]sp;cerr ln;
		int bs=bel[s],bt=bel[t];
		if(bs==bt)
		{
			rep(i,s,t) a[i]+=v;
			_merge(L[bs],s,t+1,R[bs]);
		//	_merge(L[bs],s-1,t),_merge(L[bs],t,R[bs]);
		}
		else{
			rep(i,bs+1,bt-1) tg[i]+=v;
		//	tgs[bs+1]+=v,tgs[bt]-=v,ndc=1;
			rep(i,s,R[bs]) a[i]+=v;_merge(L[bs],s-1,R[bs]);
			rep(i,L[bt],t) a[i]+=v;_merge(L[bt],t,R[bt]);
		}
	//	rep(i,1,n) cerr<<a[i]+tg[bel[i]]sp;cerr ln;
	//	rep(i,1,n) cerr<<ps[i]sp;cerr ln ln;
		return 0;
	}
...
namespace SEG{
	...
	inline int prelude(int _n)
	{
		n=_n,build(1,1,n);rep(i,1,idc) b[i].build(n);return 0;
	}
	...
	{
		rep(i,1,idc) b[i].update(l,r,v);
		return update(1,1,n,l,r,v);
	}
	inline int query(int l,int r,int v)
	{
		int ans=0;
		rep(i,1,idc) ans+=b[i].query(l,r,v);
		return ans;
	}
}
int main()
